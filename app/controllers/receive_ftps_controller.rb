class ReceiveFtpsController < ApplicationController
  ...
  def show
    @receive_ftp = ReceiveFtp.find(params[:id])
  ...
  def create
    @receive_ftp = ReceiveFtp.new(params[:receive_ftp])

    respond_to do |format|
      if @receive_ftp.save
        format.html { redirect_to(@receive_ftp, :notice => 'ReceiveFtp was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @receive_ftp.update_attributes(params[:receive_ftp])
        format.html { redirect_to(@receive_ftp, :notice => 'ReceiveFtp was successfully updated.') }
        format.xml  { head :ok }
      else
